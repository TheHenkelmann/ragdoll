...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let release_routes = Router::new()
        .route("/sources", get(get_sources).post(post_sources).put(put_sources).delete(delete_sources))
        .route("/queries", post(post_queries).get(get_queries).delete(delete_queries))
        .route("/queries/{id}", get(get_query_detail))
        .route("/chunks", get(get_chunks).post(post_chunks).delete(delete_chunks))
    ...
    let api_v1 = Router::new()
        ...
        .route("/releases/{tag}", patch(rename_release).delete(delete_release))
